...
- Added duplicate launcher detection: if an EasyCLI instance is already running in the background, launching again no longer creates another control-console process.
- A repeated launch now brings the existing background `settings` control console to the front and focuses it, including when it was hidden to the tray.
- Added the Tauri single-instance plugin and synchronized the Rust dependency lock data in both the main project and GitHub-ready source snapshot.

### `2.0.2`

- Fixed theme adaptation for the `Use API Key` dialog under the access tutorial section.
- Changed tutorial code blocks, the close-button area, bottom action bar, and scrollbars to follow the app light / dark theme instead of staying fixed black.
- Kept historical NSIS installers and generated `EasyCLI_2.0.2_x64-setup.exe`.
